...
inline void softmax(float* input, size_t num) {
  auto max_ele = *std::max_element(input, input + num);

  auto sum = std::accumulate(
      input, input + num, 0.0,
      [max_ele](float a, float b) { return std::move(a) + expf(b - max_ele); });

  auto offset = max_ele + logf(sum);
  for (auto it = input; it != (input + num); ++it) {
    *it = expf(*it - offset);
  }
}
...
static const std::vector<std::string> IMAGENET_CLASSES = {
    ...
    "feeder, snake doctor, mosquito hawk, "
    "skeeter hawk",
    "damselfly",
    "admiral",
    ...
    "machine, automated teller, automatic "
    "teller, ATM",
    "cassette",
    "cassette player",
    "castle",
...
static const std::vector<std::string> MSCOCO_CLASSES = {"background",
                                                        "person",
                                                        "bicycle",
                                                        "car",
                                                        "motorbike",
...
static const std::vector<std::array<int, 3>> MSCOCO_COLOR_CHART =
...
static const std::vector<std::array<int, 3>> VOC_COLOR_CHART =
    generateColorCharts(VOC_NUM_CLASSES);
