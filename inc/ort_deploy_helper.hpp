...
inline float sigmoid(const float x) { return 1.0 / (1.0 + expf(-x)); }

inline std::vector<uint64_t> nms(
  ...
  const uint64_t realK =
      std::max(std::min(boxesLength, topK), static_cast<uint64_t>(1));
  ...
  std::transform(std::begin(bboxes), std::end(bboxes),
                 std::back_inserter(areas), [](const auto& elem) {
...
inline std::vector<std::array<int, 3>> generateColorCharts(
    const uint16_t numClasses = 1000, const uint16_t seed = 255) {
  std::srand(seed);
  std::vector<std::array<int, 3>> colors;
  colors.reserve(numClasses);
  for (uint16_t i = 0; i < numClasses; ++i) {
    colors.emplace_back(std::array<int, 3>{std::rand() % 256, std::rand() % 256,
                                           std::rand() % 256});
  }
